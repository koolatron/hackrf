...
void usage(void)
{
	fprintf(stderr,
		...
		"\thackrf_fm ... - | play -t raw -r 24k -es -b 16 -c 1 -V1 -\n"
...
static void optimal_settings(struct fm_state *fm, int freq, int hopping)
{
	...
	if (fm->mode_demod == &fm_demod) {
		fm->output_scale = 1;}
	/* Set the frequency */
	r = hackrf_set_freq(dev, (uint32_t)capture_freq);
	if (hopping) {
		return;}
	...
	r = hackrf_set_sample_rate(dev, (uint32_t)capture_rate);
	if (r < 0) {
...
int main(int argc, char **argv)
{
	...
	if (r != 0) {
		...
		if (tuner_amp)
			fprintf(stderr, "Tuner amp enabled.\n");
		else
			fprintf(stderr, "Tuner amp disabled.\n");
	}
